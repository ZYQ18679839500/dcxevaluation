...
public interface ProjectAssignmentMapper {
    ...
    @Select("SELECT * FROM t_project_assignment WHERE " +
            ...
            "state = #{state} " +
            "ORDER BY expert_id")
    List<ProjectAssignment> queryAssignmentsByProjectIdAndStateOrderByExpertId(@Param(value = "projectId") Long projectId,
                                                                               @Param(value = "state") int state);
     ...
     * @return
     */
    @Insert("INSERT INTO t_project_assignment (project_id, expert_id, state, finish, grade,evaluation.impression) " +
            "VALUES(#{projectId}, #{expertId}, #{state}, #{finish}, #{grade},#{evaluation},#{impression})")
