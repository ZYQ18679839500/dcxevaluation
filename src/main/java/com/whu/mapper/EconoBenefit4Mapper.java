...
@Mapper
public interface EconoBenefit4Mapper {
    ...
    @Select("SELECT * FROM t_econo_benefit_4 WHERE project_id = #{projectId} " +
            "AND state = #{state} " +
            "AND expert_id = #{expertId}")
    EconoBenefit4 queryScoreByProjectIdAndState(@Param("projectId") Long projectId,
    ...
    @Select("SELECT * FROM t_econo_benefit_4 WHERE project_id = #{projectId} " +
            "AND state = #{state}")
    List<EconoBenefit4> queryScoresByProjectIdAndState(@Param("projectId") Long projectId,
                                                       @Param("state") int state);
    ...
    @Select("SELECT * FROM t_econo_benefit_4 WHERE project_id = #{projectId} " +
            "AND state = #{state} " +
            "ORDER BY expert_id")
    List<EconoBenefit4> queryScoresByProjectIdAndStateOrderByExpertId(@Param("projectId") Long projectId,
                                                                      @Param("state") int state);
     ...
     * @return 事务操作返回值
     */
    @Insert("INSERT INTO t_econo_benefit_4(project_id, expert_id, F_AssetPlan,S_AssetPlan_Para,F_AssetManagerment,S_AssetManagerment_Early,S_Managerment_Rent,S_Managerment_Increment,S_Managerment_Manager,F_Financial,S_Financial_Managerment,S_Financial_Cash,S_Financial_Operate,S_Financial_Increment, state) " +
     ...
     * @return
     */
    @Update("UPDATE t_econo_benefit_4 SET F_AssetPlan = #{F_AssetPlan}, " +
            ...
            "WHERE id = #{id}")
    int updateScore(EconoBenefit4 econoBenefit4);
     ...
     * @return
     */
    @Delete("DELETE FROM t_econo_benefit_4 WHERE id = #{id}")
    int deleteScore(EconoBenefit4 econoBenefit4);
}
