...
public interface ProjectMapper {
    ...
    @Select("SELECT * FROM t_project WHERE name = #{projectName}")
    ...
    @Update("UPDATE t_project SET " +
            "other_id = #{otherId} WHERE id = #{id}")
    int updateProjectOtherId(Project project);
    ...
    @Select("SELECT * FROM t_project WHERE assessment_state = #{state} AND prize_type=#{prizeType}")
    List<Project> queryProjectsByStateAndPrizeType(int state, int prizeType);
    ...
    @Insert("INSERT INTO t_project VALUES(#{project.id},#{prize_id}," +
            ...
            "#{project.indexNumber},#{project.additionNumber},#{project.otherId}," +
